...
def parse_tsv(pulled_schedule_str, speaker_col, topics_col):
    ...
    optionsitem = next_session[next_session.columns[topics_col]].tolist()[0] 
...
def create_message(next_session):
    ...
    message = message1 + "*What should this week's journal club presentation be about?*\n\n"
    ...
    if num_options == 1:
        ...
        full_message = message1 + "\n*Looks like there is only one offered journal club presentation this week: " + options[0] + "*\n"
        return(full_message)
...
def main():
    ...
    parser.add_argument('--topics_col', required=True, type=int, help='Zero based index for the column containing the presentation options in the dataset')
    ...
    next_session = parse_tsv(schedule, speaker_col=args.speaker_col, topics_col=args.topics_col)
